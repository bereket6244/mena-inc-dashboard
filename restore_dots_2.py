...
# 1. Add states
state_target = r"const \[toastMessage, setToastMessage\] = useState<string \| null>\(null\);"
state_new = r"""const [openActionMenuId, setOpenActionMenuId] = useState<string | null>(null);
  const [actionMenuRect, setActionMenuRect] = useState<DOMRect | null>(null);
  const [toastMessage, setToastMessage] = useState<string | null>(null);"""

if 'const [openActionMenuId' not in content:
...
with open('src/components/CustomerTab.tsx', 'w', encoding='utf-8') as f:
    f.write(content)
print("Dots restored successfully!")
