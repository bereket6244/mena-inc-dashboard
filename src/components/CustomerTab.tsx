...
The remaining balance to be paid is ${remainingBalance.toLocaleString()} birr.`;
  ...
  const executeBulkCompleteConfirmed = () => {
    ...
    const updatedList = customers.map(c => {
      if (targetIds.includes(c.id)) {
        ...
        const finalPaymentAmount = Math.max(0, totalInvoice - totalPaid);
        const completionPaymentId = `mig_rem_${c.id}`;
        const hasCompletionPayment = (c.payments || []).some(payment =>
          payment.id === completionPaymentId ||
          (
            Math.abs(Number(payment.amount || 0) - finalPaymentAmount) < 0.01 &&
            payment.date === bulkCompleteDate &&
            (payment.paymentMethodId || payment.methodId || '') === bulkCompleteBankId
          )
        );
        const payments = finalPaymentAmount > 0.01 && !hasCompletionPayment
          ? [
              ...(c.payments || []),
              {
  ...
  const getCustomerSortValue = (customer: Customer, field: CustomerColumnSortField) => {
    ...
    const isCompleted = !!(customer.deliveryDate && customer.bankRemainingId) || isCustomerPaymentComplete(customer);
  ...
  const filteredCustomers = customers.filter(c => {
    ...
    const isCompleted = !!(c.deliveryDate && c.bankRemainingId) || isCustomerPaymentComplete(c);
        ...
        >
              <thead>
                <tr className="bg-[#181818] border-b border-[#262626] text-gray-400 font-sans tracking-wider uppercase text-center">
                  ...
                  <SortableCustomerHeader field="deliveryDate" className="min-w-[150px] bg-[#31111E]/5">Delivery / Status</SortableCustomerHeader>
                </tr>
              </thead>
              <tbody className="divide-y divide-[#262626] text-gray-300">
                {isLoading ? (
                  <>
                    {Array.from({ length: 7 }).map((_, rowIndex) => (
                      ...
                      >
                        ...
                        {/* 7. Delivery / Status */}
                        <td className="py-1.5 px-2 border-r border-[#262626] font-sans align-top bg-[#1c1c1c]/10 min-w-[150px] max-w-[170px]">
                          <div className="flex flex-col gap-1.5 pt-1">
                            {/* Delivery Date */}
                            <div className="min-w-0 w-full">
                              <span className="block text-[8px] uppercase tracking-wider text-gray-500 leading-tight">Delivery Date</span>
                              <div className="w-full h-[21px] rounded-md skeleton-line" style={{ '--w': '128px' } as React.CSSProperties} />
                            </div>
                            {/* Incomplete Reason */}
                            <div className="min-w-0 w-full">
                  ...
                  const isCompletedByPayment = fullVal > 0 && totalPaidAmount >= fullVal - 0.01;
                  const isCompleted = !!(c.deliveryDate && c.bankRemainingId) || isCompletedByPayment;
                        ...
                        <div className="space-y-1">
                          ...
                          <div className="flex items-center gap-1 pt-0.5 font-sans">
                            ...
                            <button
                              ...
                              className="customer-row-payment-action text-gray-400 hover:text-[#71b536] hover:bg-[#262626] p-1.5 rounded-md transition-colors cursor-pointer"
                              title="Add Payment"
                            >
                            ...
                            <button
                              ...
                              className="customer-row-increase-action text-gray-400 hover:text-[#ee317b] hover:bg-[#262626] p-1.5 rounded-md transition-colors cursor-pointer"
                      ...
                      <td className="py-1.5 px-2 border-r border-[#262626] font-sans align-top bg-[#1c1c1c]/10 min-w-[150px] max-w-[170px]">
                          <div className="flex flex-col gap-1.5 pt-1">
                            <div className="min-w-0 w-full">
                              <span className="block text-[8px] uppercase tracking-wider text-gray-500 leading-tight">Delivery Date</span>
                              <span
                                className="block w-full max-w-[118px] rounded-md border border-[#262626] bg-[#121212] px-1.5 py-0.5 text-[11px] leading-tight text-sky-400"
                                title={displayedDeliveryDate ? 'Latest payment date' : 'Order is not complete'}
                              >
                                {displayedDeliveryDate ? formatDateFriendly(displayedDeliveryDate) : ''}
                              </span>
                          </div>
                          <div className="min-w-0 w-full">
                            <span className="block text-[8px] uppercase tracking-wider text-gray-500 leading-tight">Incomplete Reason</span>
                            <input
                              ...
                              className="bg-[#121212] text-[11px] text-gray-350 hover:text-white border border-[#262626] hover:border-[#ee317b] focus:border-[#ee317b] outline-none px-1.5 py-0.5 font-sans rounded-md w-full leading-tight disabled:opacity-60 disabled:cursor-not-allowed"
              ...
              <div 
                ...
                className={`border rounded-xl p-3.5 shadow-sm flex flex-col justify-between transition-all duration-300 ${isSearchHighlighted ? 'global-search-highlight' : ''} ${
                  isSelected
                    ...
                    : isOverPaid
                      ? 'bg-yellow-200 dark:bg-[#312b12] border-yellow-500/80 text-yellow-950 dark:text-yellow-100'
                      : isExactlyPaid
                        ? 'bg-green-100 dark:bg-[#132c10] border-green-500/80 text-green-950 dark:text-green-100'
                        : isCompleted
              ...
              >
                <div className="space-y-2.5">
                  ...
                  <div className="grid grid-cols-2 divide-x divide-[#262626]/50 border-t border-[#262626] mt-3 pt-3 text-xs font-sans">
                    ...
                    <div className="flex flex-col gap-1 px-3.5">
                      ...
                      <span
                        className="bg-[#161616] text-white border border-[#262626] rounded-md px-1 h-7 text-[10px] font-sans font-bold w-full mt-1 flex items-center justify-center text-center"
                        title={displayedDeliveryDate ? 'Latest payment date' : 'Order is not complete'}
                      >
                        {displayedDeliveryDate ? formatDateFriendly(displayedDeliveryDate) : ''}
                      </span>
                ...
                <div className="grid grid-cols-3 gap-1.5 mt-3.5 pt-3 border-t border-[#262626]">
                  <button
                    type="button"
                    onClick={() => setManagingPaymentsFor(c)}
                    className="customer-row-payment-action text-[10px] border border-green-900/40 hover:bg-green-950/10 text-green-500 font-sans font-bold py-1.5 px-0.5 rounded-lg transition-all flex items-center justify-center gap-1 cursor-pointer bg-transparent"
                  >
                  ...
                  <button
                    type="button"
                    onClick={() => setManagingAdjustmentsFor(c)}
                    className="customer-row-increase-action text-[10px] border border-pink-900/40 hover:bg-pink-950/10 text-pink-500 font-sans font-bold py-1.5 px-0.5 rounded-lg transition-all flex items-center justify-center gap-1 cursor-pointer bg-transparent"
                  >
      ...
      <AnimatePresence>
        {managingPaymentsFor && (() => {
          ...
          const visiblePaymentHistory = (managingPaymentsFor.payments || []).filter(payment => {
            ...
            const paymentBankId = payment.paymentMethodId || payment.methodId || '';
            return !(
              payment.id === legacyAdvancePaymentId ||
              (
                Math.abs(Number(payment.amount || 0) - legacyAdvanceAmount) < 0.01 &&
                paymentBankId === legacyAdvanceBankId &&
                payment.date === legacyAdvanceDate
              )
            );
          });
          ...
          >
            ...
            >
              ...
              <div className="customer-payment-modal-body flex-1 overflow-y-auto p-4 custom-scrollbar">
                <AnimatePresence>
                  {showPaymentHistory && (
                    <motion.div 
                    ...
                    >
                      ...
                      <div className="space-y-2">
                        {legacyAdvanceAmount > 0 && (
                          <div className="customer-payment-history-item flex justify-between items-center bg-gray-50 dark:bg-[#1a1a1a] p-3 rounded-lg border border-gray-200 dark:border-[#2a2a2a]">
                            <div>
                              <div className="customer-payment-history-amount text-gray-900 dark:text-white text-sm font-bold flex items-center gap-2">
                                {formatMoney(legacyAdvanceAmount, managingPaymentsFor.currency)}
                                {legacyAdvanceBankId && (
                                  <span className="customer-payment-method-badge text-[10px] font-normal bg-gray-100 dark:bg-[#262626] text-gray-600 dark:text-gray-300 px-1.5 py-0.5 rounded">
                                    {bankAccounts.find(b => b.id === legacyAdvanceBankId)?.name || 'Unknown Bank'}
                                  </span>
                                )}
                              </div>
                              <div className="text-gray-500 text-xs">{formatDateFriendly(legacyAdvanceDate)}</div>
                              ...
                              <button
                                ...
                                onClick={() => {
                                  const updatedCustomer = {
                                    ...
                                    advancePaymentDate: '',
                                    paymentMethodId: '',
                                    payments: (managingPaymentsFor.payments || []).filter(payment => payment.id !== legacyAdvancePaymentId)
                                  };
                                ...
                                className="p-1.5 text-gray-400 hover:text-[#F87171] hover:bg-red-950/20 rounded transition-colors"
                            ...
                            <div key={p.id} className="customer-payment-history-item flex justify-between items-center bg-white dark:bg-[#1a1a1a] p-3 rounded-lg border border-gray-200 dark:border-[#2a2a2a]">
                              ...
                              <div className="flex items-center gap-3">
                                ...
                                <button
                                  type="button"
                                  onClick={() => {
                                    const updatedPayments = (managingPaymentsFor.payments || []).filter(payment => payment.id !== p.id);
                                    const updatedCustomer = {
                                      ...managingPaymentsFor,
                ...
                <div className="customer-payment-form bg-white dark:bg-[#1a1a1a] p-4 rounded-xl border border-gray-200 dark:border-[#2a2a2a]">
        ...
        {managingAdjustmentsFor && (
          ...
          >
            ...
            >
              ...
              <div className="customer-adjustment-modal-footer p-4 border-t border-gray-200 dark:border-[#333] bg-gray-50 dark:bg-[#0a0a0a] flex justify-end gap-2">
                <button
                ...
                <button
                  ...
                  className="customer-adjustment-save-button px-4 py-2 text-xs font-bold bg-[#ee317b] hover:bg-[#c62060] text-white rounded transition-colors cursor-pointer"
                >
                  Save Addition
                </button>
              </div>
